...
import com.supraliminalsolutions.spamblocker.activity.TrackerActivity;
...
public class SMSOps {
    ...
    public void checkSimChange() {
        String number = DataManager.getInstance(context).checkSimIfChanged();
        if (!number.equals("")) {
    ...
    public void inform() {
        try {
            ...
            ArrayList<String> numbers = DataManager.getInstance(context).getAllSupportNumbers();
